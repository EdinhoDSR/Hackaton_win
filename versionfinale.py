...
def simplifierCVKeyWords(textCV):
    ...
    for i in textCV:
        ...
        for keyword in mostImportantKeyWords:
            if i.find(keyword) != -1:
                chaine += i
                chaine += "\n"
                break
...
def constructor_bouton(element):
    global fenetre2
    b = Button(fenetre2, text=element, command=lambda element=element: ouvrirpdf(element))
    return b
...
class Example(Frame):
    ...
    def __init__(self, parent):
        ...
        vsb = Scrollbar(fenetre2, orient="vertical", command=text.yview)
        ...
        for element in scoresKeys:
            parsed = parser.from_file(path + '/' + element)
            b = constructor_bouton(element)
            label = Label(fenetre2, text=simplifierCVKeyWords(parsed))
...
def afficherCoefficient(frame, string, double, i):
    ...
    scale = Scale(ligne, orient='horizontal', from_=0, to=100, resolution=0.1, length=150,
                  command=lambda i=2, nom=string: actualiser(nom, i))
    scale.set(double)
...
def demanderNouveauMot():
    ...
    poids = DoubleVar()
    scale = Scale(entree, variable=poids, orient='horizontal', from_=0, to=100, resolution=0.1,
                  label='Importance du mot-clé', length=250)
    scale.set(10)
    boutonAjouter = Button(entree, text="Ajouter",
                           command=lambda: afficherCoefficient(tableau, valeur.get(), scale.get(), nbCoeffs))
...
# panel1 = Label(fenetre, image=photo)
...
label.pack(pady=10, side=TOP,padx =10)
...
canva = Canvas(mainCadreGauche)
canva.pack(fill='both', expand=True)
tableau = Frame(canva)
window = canva.create_window(0, 0, anchor=NW, window=tableau)
tableau.pack()

bouton = Button(mainCadreGauche, text="+", command=demanderNouveauMot, width=5, height = 2)
bouton.pack(side=BOTTOM, padx=15, pady=5)
