...
def asset_cvs():
    ...
    for i in range(len(scoresKeys)):
        ...
        scoreLabels[i].config(text=scoresKeys[i] + "    " + str(scores[scoresKeys[i]]), command = lambda element=scoresKeys[i]: ouvrirpdf(element))
    augustin()
...
def demanderPath():
    ...
    for file in getCVName(os.listdir(path)):
        scoreLabel = Button(mainCadreDroit, text="", font=("Helvetica", "12"), )
        scoreLabel.grid(row=r, column=3, padx=10, pady=5)
        scoreLabels.append(scoreLabel)
        r += 1
    importerDico()
